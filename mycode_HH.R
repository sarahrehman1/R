...
t <- stack(elevationcrop_sw, elevationcrop_sw, quick=TRUE)
r1 <-elevationcrop_sw
resample(bioclimVars,elevationcrop_sw, method='near')
r2 <- crop(bioclimVars,extent(elevationcrop_sw))
r22 <- raster(vals=values(r2),ext=extent(r1),crs=crs(r1),
              nrows=dim(r1)[1],ncols=dim(r1)[2])
t
t <-resample(all,elevationcrop_sw, method="near")
?`resample,Raster,Raster-method`

str(all)
str(elevationcrop_sw)
